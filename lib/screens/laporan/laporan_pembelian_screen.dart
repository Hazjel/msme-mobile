...
class _LaporanPembelianScreenState extends State<LaporanPembelianScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final data = await _service.laporanPembelian(
        start: _fmt(_start),
        end: _fmt(_end),
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Total Pembelian: ${formatRupiah(_data!['total'])}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  Text('Jumlah Transaksi: ${_data!['jumlah_transaksi']}'),
                ]),
              ),
            ),
            const SizedBox(height: 8),
            ...(_data!['data'] as List).map((p) {
